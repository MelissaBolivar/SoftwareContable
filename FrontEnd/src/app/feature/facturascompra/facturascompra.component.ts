...
import { FormularioFacturasCompraComponent } from './component/formulario-facturascompra/formulario-facturascompra.component';
import { FacturasCompra } from '../../shared/interfaces/facturascompra.interface';
import { FacturasCompraService } from '../../shared/Service/facturascompra/facturascompra.service';

@Component({
  selector: 'app-facturascompra',
  standalone: true,
  templateUrl: './facturascompra.component.html',
  styleUrls: ['./facturascompra.component.scss'],
  imports: [
    FormsModule,
    CommonModule,
    ReactiveFormsModule,
...
export class FacturasCompraComponent implements OnInit {
  ...
  constructor(
    private readonly service: FacturasCompraService,
  ...
  delete(infoComponent: FacturasCompra): void {
  ...
  new(): void {
    this.ref = this.dialogService.open(FormularioFacturasCompraComponent, {
      data: { action: 'Crear' },
      width: '90%',
      height: '100%',
  ...
  edit(infoComponent: FacturasCompra): void {
    this.ref = this.dialogService.open(FormularioFacturasCompraComponent, {
      data: {
        action: 'Actualizar',
  ...
  verFactura(infoComponent: FacturasCompra): void {
    this.ref = this.dialogService.open(FormularioFacturasCompraComponent, {
      data: {
        action: 'ver',
        id: infoComponent.facturaId,
  ...
  getTotal(infoComponent: FacturasCompra): string {
