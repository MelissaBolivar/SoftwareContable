...
import { CreateOrUpdateFacturasCompra } from '../../../../shared/interfaces/CreateOrUpdateFacturasCompra.interface';
...
@Component({
  selector: 'app-formulario-facturascompra',
  standalone: true,
  templateUrl: './formulario-facturascompra.component.html',
  styleUrls: ['./formulario-facturascompra.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
...
export class FormularioFacturasCompraComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getTipoFactura().subscribe(data => {
      this.tipoFactura = data
        ...
        .filter(f => f.nombre?.toLowerCase() === 'compra');
  ...
  guardar(): void {
    ...
    const formData = raw;
    debugger;
    const payload: CreateOrUpdateFacturasCompra = {
