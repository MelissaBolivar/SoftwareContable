...
import { FormularioComprobantecajaComponent } from './component/formulario-comprobantecaja/formulario-comprobantecaja.component';
import { Comprobantecaja } from '../../shared/interfaces/comprobantecaja';
import { ComprobantecajaService } from '../../shared/Service/comprobantecaja/comprobantecaja.service';

@Component({
  selector: 'app-comprobantecaja',
  standalone: true,
  templateUrl: './comprobantecaja.component.html',
  styleUrls: ['./comprobantecaja.component.scss'],
  imports: [
    FormsModule,
    CommonModule,
    ReactiveFormsModule,
...
export class ComprobantecajaComponent implements OnInit {
  infoTable: Comprobantecaja[] = [];
  loading = true;
  ...
  constructor(
    private readonly service: ComprobantecajaService,
  ...
  delete(infoComponent: Comprobantecaja): void {
  ...
  new(): void {
    this.ref = this.dialogService.open(FormularioComprobantecajaComponent, {
      data: { action: 'Crear' },
      width: '90%',
      height: '100%',
  ...
  edit(infoComponent: Comprobantecaja): void {
    this.ref = this.dialogService.open(FormularioComprobantecajaComponent, {
      data: {
        action: 'Actualizar',
  ...
  verFactura(infoComponent: Comprobantecaja): void {
    this.ref = this.dialogService.open(FormularioComprobantecajaComponent, {
      data: {
        action: 'ver',
        id: infoComponent.facturaId,
  ...
  getTotal(infoComponent: Comprobantecaja): string {
