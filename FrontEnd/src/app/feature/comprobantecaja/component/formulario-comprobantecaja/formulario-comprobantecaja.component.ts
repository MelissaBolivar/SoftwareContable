...
import { CreateOrUpdateComprobanteCaja } from '../../../../shared/interfaces/CreateOrUpdateComprobanteCaja.interface';
...
@Component({
  selector: 'app-formulario-comprobantecaja',
  standalone: true,
  templateUrl: './formulario-comprobantecaja.component.html',
  styleUrls: ['./formulario-comprobantecaja.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  providers: [ProveedorService, MessageService]
})
export class FormularioComprobantecajaComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getTipoFactura().subscribe(data => {
      this.tipoFactura = data
        ...
        .filter(f => f.nombre?.toLowerCase() === 'comprobante de caja');
...
/*
    if (tipoFacturaSeleccionado?.nombre?.toLowerCase() !== 'compra') {
      this.messageService.add({
        severity: 'warn',
        summary: 'Tipo de Factura inválido',
        detail: 'Solo se permite guardar facturas de tipo Compra.'
      });
      return;
    }
*/
