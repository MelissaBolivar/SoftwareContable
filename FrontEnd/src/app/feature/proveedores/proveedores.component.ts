...
@Component({
  ...
  providers:[
    DialogService,
    ConfirmationService,
    // ProveedorService se provee en root; no debe listarse aquí
    HttpService,
    UserService,
    MessageService
  ],
...
export class ProveedoresComponent implements OnInit {
  ...
  delete(infoComponent: Proveedor) {
...
onEstadoFilter(event: Event): void {
  ...
  private updateLocalEstado(terceroId: number, activo: boolean): void {
    ...
    if (idxAll !== -1) {
      this.allInfo[idxAll] = { ...this.allInfo[idxAll], Activo: activo, estado: activo } as Proveedor;
    }

    const idxView = this.infoTable.findIndex(i => Number((i as any).TerceroId ?? (i as any).terceroId) === terceroId);
    if (idxView !== -1) {
      this.infoTable[idxView] = { ...this.infoTable[idxView], Activo: activo, estado: activo } as Proveedor;
    }
  }

  // --- métodos para activar / desactivar (llaman al servicio y actualizan UI) ---

  desactivar(infoComponent: Proveedor): void {
    ...
    if (typeof this.service.deactivate === 'function') {
      this.service.deactivate(id).subscribe({
        next: () => {
          this.showMessage('success', 'Proveedor', 'Proveedor Desactivado Correctamente');
          this.updateLocalEstado(id, false);
        },
        error: () => this.showMessage('error', 'Error', 'No se pudo desactivar el proveedor')
      });
      return;
    }
  ...
  reactivar(infoComponent: Proveedor): void {
    const id = Number((infoComponent as any).TerceroId ?? (infoComponent as any).terceroId);
    if (isNaN(id)) return;
    ...
    if (typeof this.service.reactivate === 'function') {
      this.service.reactivate(id).subscribe({
        next: () => {
          this.showMessage('success', 'Proveedor', 'Proveedor Activado Correctamente');
          this.updateLocalEstado(id, true);
        },
        error: () => this.showMessage('error', 'Error', 'No se pudo reactivar el proveedor')
      });
      return;
    }
